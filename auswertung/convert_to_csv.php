...
while($rowPerson = mysql_fetch_assoc($queryPerson)) {
	$line = array(
		$rowPerson['id'], $rowPerson['bogen_id'
	);
	$line[] = $rowP
}

function query_property($person_id, $property) {
	$query = mysql_query("SELECT value FROM t_person_properties WHERE person_id = ".intval($person_id)." AND property='".mysql_real_escape_string($property).' LIMIT 1');
