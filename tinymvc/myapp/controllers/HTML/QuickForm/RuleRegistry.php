...
class HTML_QuickForm_RuleRegistry
{
    ...
    {
        list($clazz, $path) = $GLOBALS['_HTML_QuickForm_registered_rules'][$ruleName];

        if (!isset($this->_rules[$clazz])) {
            if (!empty($path)) {
                include_once($path);
            }
            $this->_rules[$clazz] =& new $clazz();
        }
        $this->_rules[$clazz]->setName($ruleName);
        return $this->_rules[$clazz];
    } // end func getRule
